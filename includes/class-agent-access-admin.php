...
class Agent_Access_Admin {
	...
	public function render_profile_section( $user ) {
		...
		<div id="agent-access" class="agent-access-profile-section">
			<h2 class="agent-access-title">
				...
				<?php esc_html_e( 'BotCreds', 'agent-access' ); ?>
			...
			<nav class="nav-tab-wrapper">
				<a href="<?php echo esc_url( add_query_arg( 'tab', 'connections', menu_page_url( 'agent-access', false ) ) ); ?>"
				   class="nav-tab <?php echo 'connections' === $current_tab ? 'nav-tab-active' : ''; ?>">
					<?php esc_html_e( 'Connections', 'agent-access' ); ?>
				</a>
				<a href="<?php echo esc_url( add_query_arg( 'tab', 'activity', menu_page_url( 'agent-access', false ) ) ); ?>"
				   class="nav-tab <?php echo 'activity' === $current_tab ? 'nav-tab-active' : ''; ?>">
					<?php esc_html_e( 'Activity Log', 'agent-access' ); ?>
				</a>
			</nav>
	...
	private function render_activity_log_tab() {
		...
		<form method="get" style="margin: 1em 0;">
			...
			<select name="source">
				<option value=""><?php esc_html_e( 'All sources', 'agent-access' ); ?></option>
				<option value="agent-access" <?php selected( $source, 'agent-access' ); ?>><?php esc_html_e( 'Agent Access', 'agent-access' ); ?></option>
				<option value="wordpress-mcp" <?php selected( $source, 'wordpress-mcp' ); ?>><?php esc_html_e( 'WordPress.com MCP', 'agent-access' ); ?></option>
			</select>

			<select name="method">
				<option value=""><?php esc_html_e( 'All methods', 'agent-access' ); ?></option>
			...
			<?php submit_button( __( 'Filter', 'agent-access' ), 'secondary', '', false ); ?>
		</form>
		...
		<p>
			...
			printf(
				...
				esc_html__( '%d total log entries.', 'agent-access' ),
				(int) $total
			);
			?>
		</p>
		...
		<?php if ( empty( $entries ) ) : ?>
			<p><em><?php esc_html_e( 'No activity logged yet.', 'agent-access' ); ?></em></p>
		<?php else : ?>
			<table class="widefat striped" style="font-size:13px;">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Time', 'agent-access' ); ?></th>
						<th><?php esc_html_e( 'Source', 'agent-access' ); ?></th>
						<th><?php esc_html_e( 'User', 'agent-access' ); ?></th>
						<th><?php esc_html_e( 'Method', 'agent-access' ); ?></th>
						<th><?php esc_html_e( 'Route', 'agent-access' ); ?></th>
						<th><?php esc_html_e( 'Object', 'agent-access' ); ?></th>
						<th><?php esc_html_e( 'IP', 'agent-access' ); ?></th>
					</tr>
				</thead>
				<tbody>
